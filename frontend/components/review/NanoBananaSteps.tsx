...
// Category (garment / shoes / bag / accessory) + the step labels per category
// live in lib/nbCategory.ts — the same vocabulary the backend mirrors to pick
// the prompt set, and the one guessProductType() uses for the type token.
import { nbCategory, STEPS_BY_CATEGORY } from "@/lib/nbCategory";
    ...
    const calls = Array.from({ length: TOTAL_VARIANTS }, async (_, i) => {
      try {
        const res = await higgsfieldQueue.run(() => api.higgsfield({
          prompt_type: stepNum,
          product_type: data.productType || "garment",
